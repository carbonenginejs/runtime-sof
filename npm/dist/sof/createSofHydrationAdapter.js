      ...
      if (context?.kind === "EveSpaceObjectDecal") {
        ...
        if (setup && typeof instance.AddMesh === "function") {
          ...
          for (const mesh of setup.meshes ?? []) {
            instance.AddMesh(mesh.geometryPath, mesh.castsShadow, mesh.reflectionMode, mesh.meshIndex, mesh.areas, mesh.instanceTransforms, mesh.sofHullName, mesh.sofLocatorSetName);
          }
        }
        ...
        if (setup && typeof instance.Add === "function") {
          ...
          for (const item of setup.instances ?? []) {
            instance.Add(item.transform, item.functionality, item.hasTrail, item.atlasIndex0, item.atlasIndex1, item.lightScale);
          }
        }
        instance.Initialize?.();
